...
// head에서 시작하여 NULL을 만날때까지 계속 next하며 개수 세는 함수
void lengthCheck(Node* headPointer) {
    int lenLL = 0;
    for(; headPointer != NULL ; headPointer = headPointer->next )
    {
        lenLL = lenLL+1;
...
int main()
{
    ...
    Node* insertedNode6 = insertNode(&headPointer, headPointer, 60);
    printListPointer(headPointer);
    lengthCheck(headPointer);

    deleteNode(&headPointer, insertedNode1);
    deleteNode(&headPointer, insertedNode2);
    deleteNode(&headPointer, insertedNode3);
    deleteNode(&headPointer, insertedNode4);
